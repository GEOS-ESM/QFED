...
def viirs_pixel_area(sample, file='pix_size_viirs_iband.txt'):
    '''
    Reads a file containing the along scan and along track pixel 
    dimensions (in km) as a function of sample number or scan angle 
    for the VIIRS 375m data.
    Note the corresponding image is 6400 samples wide while the file 
    gives you 3200 values since pixels grow from nadir to the edge of 
    the image creating a mirror effect. In this case, sample = 0 at nadir 
    and sample = 3199 at the edge (either left or right) of the image.
    '''

    data_description = {'names': ('sample', 'along_scan', 'along_track', 'scan_angle'),
                        'formats': ('i4', 'f4', 'f4', 'f4')}

    data = np.loadtxt(file, dtype=data_description, delimiter=',', skiprows=1)
    
    smpl = data['sample']
    _area = data['along_scan'] * data['along_track']

    # mirror area
    # note: if nadir is _area[0], then
    # we could do 
    #     1) area = np.concatenate((_area[::-1], _area[1:])
    # or 
    #     2) area = np.concatenate((_area[:0:-1], _area[:])
    # however the length of the array would be 6400-1!?
    #
    # looking at the scan angle values (they are > 0),
    # one needs to likely mirror the entire array, i.e.
    #      3) area = np.concatenate((_area[::-1], _area[:]))
    # this will construct an array with length 6400

    # this should work in a way similar to (1), 
    # however np.int16 needs to be hardcoded to
    # avoid underflow (sample is of type uint16) when sample - 3199 < 0
    #   sample = np.abs(sample - 3199, dtype=np.int16)
    #   return _area[sample]
    # or the equivalent but slower
    #   area = [_area[np.abs(i - 3199)] for i in sample]
    #   return area

    # equivalent to 3) but possibly more fragile and less readable
    #      4) area = _area[np.abs(np.array(sample - 3199.5, dtype=np.int16))]
    # ...go with 3)
