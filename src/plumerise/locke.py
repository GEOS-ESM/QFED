...
class MINXs_PR(MINXs):
    ...
    def getPlume1(self,i,bfac=15.,ffac=None,hfac=None):
        ...
        On input,
           ...
           ffac  ---  if not None, use frp to increase bstar
	...
	"""
        ...
        # Derive (scaled) FRP contribution
        # --------------------------------
        if ffac!=None:
            if self.frp[i]<=0:
                raise ValueError, "Invalid frp=0 for i=%d"%i
            # Note: Assuming FRP is not per unit area (MISR doc says units are MW)
            bfire = MAPL_GRAV / (self.sample.rhoa[i]*ustar) \
                    * self.frp[i] / (MAPL_CP * self.sample.tsh[i])
            bstar = bfac * self.sample.bstar[i] + ffac * bfire

        # Derive (scaled) fire Heat Flux contribution
        # -------------------------------------------
        elif hfac!=None:
            # Heat flux is in kW/M2
            bfire = MAPL_GRAV / (self.sample.rhoa[i]*ustar) \
                    * 1000. * self.hflux[i] / (MAPL_CP * self.sample.tsh[i])
            bstar = bfac * self.sample.bstar[i] + hfac * bfire

        # Simply scale model bstar
        # ------------------------
        else:     
            bstar = bfac # * self.sample.bstar[i] 
