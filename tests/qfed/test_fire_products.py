...
class test(unittest.TestCase):
    def test_basic_functionality(self):
        ...
        for (instrument, satellite), file in queue.items():
            ...
            if n_fires > 0:
                print(f'fire lon = {reader.get_fire_longitude(file)}')
                print(f'fire lat = {reader.get_fire_latitude(file)}')
                print(f'fire frp = {reader.get_fire_frp(file)}')
                print(f'fire pixel area = {reader.get_fire_pixel_area(file)}')
