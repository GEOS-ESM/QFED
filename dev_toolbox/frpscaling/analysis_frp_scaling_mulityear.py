...
if __name__ == "__main__":
    ...
    # Analysis years
    year_range = (cfg["year_range"]["start"], cfg["year_range"]["end"])
    ...
    print(f"QFED base path     : {base_path}")
    ...
    print(f"Computing scaling factors across all years ({year_range[0]}-{year_range[1]})...")
    print("=" * 80)
    ...
    if baseline_satellite not in combined_all_data:
        ...
        print(f'\n - Combined period: {year_range[0]}-{year_range[1]}')
        ...
        for sat in satellites_to_process:
            ...
            for biome in biomes:
                ...
                try:
                    ...
                    print(f" - {biome:<{12}} "f"{logc:^{15}.6f} "f"{scaling_factor:^{15}.6f} ")
        ...
        for sel_sat in satellites_to_process:
            ...
            try:
                ...
                for i, biome in enumerate(biomes):
                    ...
                    axes[i].set_title(f"{panel_letter} {prefix} {biome_map[biome]} ({year_range[0]}-{year_range[1]})\nScaling: {all_scale_dicts[sel_sat][biome]:4.4f}")

                plt.savefig(f'{figure_dir}/SCATTER_Scaling_factor_{sel_sat}_{year_range[0]}-{year_range[1]}.png', dpi=300)
                plt.close()
        ...
        for sat in satellites_to_process:
            ...
            try:
                ...
                for idx, biome in enumerate(biomes):
                    ...
                    axes[idx].set_title(f"{panel_letter} {prefix} {product} - {biome_map[biome]} ({year_range[0]}-{year_range[1]})")
                ...
                plt.savefig(f'{figure_dir}/TS_FPR_Density_{product}_{year_range[0]}-{year_range[1]}.png', dpi=300)
                plt.close()
    ...
    print("\nGenerated files:")
    for sat in satellites_to_process:
        print(f"  - {figure_dir}/SCATTER_Scaling_factor_{sat}_{year_range[0]}-{year_range[1]}.png")
        print(f"  - {figure_dir}/TS_FPR_Density_{sat.upper()}_{year_range[0]}-{year_range[1]}.png")
